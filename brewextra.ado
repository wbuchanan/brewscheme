...
prog def brewextra, rclass
	...
	preserve
		...
		} // End ELSE Block when no arguments passed to files parameter
...
prog def checkfilespec, rclass
	...
	preserve
		...
		else if `isfilenm' == 1 & `"`infiles'"' != "" {
		
			// Sets the nfnm local based on a fully qualified file name
			mata: st_local("nfnm", pathbasename(st_local("infiles")))
		
		} // End ELSEIF Block for fully qualified file path
