...
prog def cbinvert, rclass
	...
	copy "http://mkweb.bcgsc.ca/colorblind/colorblind.rgb.table.txt.gz"		 ///   
		 "colorblind.rgb.table.txt.gz"
	...
	if regexm(`"`c(os)'"', "[xX]") != 1 {
		...
		"to decompress the file colorblind.rgb.table.txt.gz manually."		 ///   
		"Enter the letter q into the console and hit enter after doing this" ///   
	...
	// For *nix based systems
		...
		// Decompress the file	 
		! gunzip colorblind.rgb.table.txt.gz
		
	} // End ELSE Block for *nix based systems
	
	// Read the data into Stata
	qui: infix str lne 1-2045 using colorblind.rgb.table.txt, clear

	// Add notes to dataset
	note "Data from : http://mkweb.bcgsc.ca/colorblind/"
	note "Data Author: Martin Krzywinski"
	note `"`dltime'"'
	note "Implemented in Stata using brewscheme"
	note "BBuchanan@mde.k12.ms.us - brewsceme maintainer"
	...
	// Save the dataset
	qui: save `"`c(sysdir_personal)'b/brewcolorblind.dta"', replace
